CREATE OR REPLACE FUNCTION import_manifest_batch(items json)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
...
BEGIN
  ...
  LOOP
    ...
    -- Update image_confidence
    UPDATE products SET image_confidence = confidence_val WHERE numref = sku_val;
    IF NOT FOUND THEN
...
GRANT EXECUTE ON FUNCTION import_manifest_batch(json) TO anon, authenticated;
